...
template<class Domain>
std::istream& Poly1Dom<Domain,Dense>::read ( std::istream& i, Rep& P, bool flag) const //Pooja for easy input
{
    if(flag)
      return Poly1Dom<Domain,Dense>::read(i,P);
    long deg;
    i >> deg;
    init(P,Degree(deg));
// JGD 18.09.2002
    long index;
    i>>index;
    while(index>-1&&index<=deg)
    {
      _domain.read( i, P[(size_t)index]);
      i>>index;
    }
